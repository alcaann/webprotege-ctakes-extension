...
echo "1. Open a terminal and run: cd /webprotege && mvn gwt:codeserver -Dgwt.persistentunitcache=true -Dgwt.persistentunitcachedir=/webprotege/.gwt_cache -Dgwt.draftCompile=true -Dgwt.localWorkers=4 -Dgwt.extraJvmArgs=-Dgwt.rpc.xmlserialization=false"
echo "2. Open another terminal and run: cd /webprotege && mvn -Denv=dev tomcat7:run -Dgwt.extraJvmArgs=-Dgwt.rpc.xmlserialization=true"
