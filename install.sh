...
if ! command -v nc &>/dev/null; then
    echo "netcat not available, trying alternative connectivity check..."
    # Use timeout with /dev/tcp as alternative
    if timeout 5 bash -c "</dev/tcp/$MONGO_HOST/27017" 2>/dev/null; then
